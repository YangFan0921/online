...
describe('SSR: renderToString', () => {
  ...
  it('dynamic style', done => {
    renderVmWithOptions({
      ...
      expect(result).toContain(
        '<div data-server-rendered="true" style="background-color:black;fonts-size:14px;color:red;"></div>'
      )
      done()
    })
  })
  ...
  it('nested custom component style', done => {
    renderVmWithOptions({
      ...
      expect(result).toContain(
        '<div data-server-rendered="true" style="text-align:left;fonts-size:520rem;color:red;"></div>'
      )
      done()
    })
  })
